...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Gen Varela - gmv1758@rit.edu"; //EDITED
...
void Application::Display(void)
{
	...
	*/
	...
	static std::vector<vector3> v3Stop; //EDITED - I tried moving this to AppClass.h but it broke
...
#pragma region
	//EDITED - mostly from 15_Rotations but some things are changed (animation time and axis)
		//matrix4 m4Rotation = glm::rotate(IDENTITY_M4, m_v3Orientation.x, vector3(1.0f, 0.0f, 0.0f));  
	if (rotating) //asks AppClass.h if AppClassControls.cpp has changed the value of rotating, and if it has, do this:
	{
		matrix4 m4ModelRot; //a new rotation matrix
		matrix4 m4Rotation = glm::rotate(IDENTITY_M4, fTimer * 180.0f, vector3(1.0f, 0.0f, 0.0f)); 
		for (uint i = 0; i < 2500; ++i)
			m4ModelRot = m4Rotation * glm::translate(IDENTITY_M4, vector3(2.5f, 0.0f, 0.0f)) * glm::transpose(m4Rotation);
		...
		m4ModelRot = m4TransInverse * m4Rotation * m4Translation; //combines these transformation matrices into one

		m4Model *= m4ModelRot; //multiply it to the matrix that gets passed in below
	}
#pragma endregion
