...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	//EDITED
	static bool toggleRot = false; //Toggles Rotation on and off- local to this class; only R key release changes it
	...
	//EDITED
	case sf::Keyboard::R:
		toggleRot = !toggleRot; //When R is released, the toggle switches over 
		if (toggleRot) //if the toggle is on, 
		{
			rotating = true; //then the mesh is rotating (rotating lives in AppClass.h)
		}
		else //otherwise,
		{
			rotating = false; //the mesh isn't rotating
		}
		break;
